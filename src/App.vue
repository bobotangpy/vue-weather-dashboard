          ...
          <div id="info">
            ...
            <div class="wrapper-right">
              ...
              <div class="location-info">
                <div
                  id="location-desc"
                  class="col-md-3 col-sm-4 col-xs-12 pl-0 pr-0"
                >
                  <img
...
export default {
  ...
  watch: {
    rawWeatherData: function () {
      if (this.rawWeatherData) {
        ...
        geocoder.geocode(
          ...
          (result) => {
            ...
            if (result) {
              ...
              this.long = result.items[0].position.lng;
              this.currentWeather.full_location = this.full_location =
                result.items[0].title;

              // Remember to beautify lat for N/S
              if (this.lat > 0) {
                this.currentWeather.formatted_lat =
                  (Math.round(this.lat * 10000) / 10000).toString() + "°N";
              } else if (this.lat < 0) {
                this.currentWeather.formatted_lat =
                  (-1 * (Math.round(this.lat * 10000) / 10000)).toString() +
                  "°S";
              } else {
                this.currentWeather.formatted_lat = (
                  Math.round(this.lat * 10000) / 10000
                ).toString();
              }
              // Remember to beautify long for N/S
              if (this.long > 0) {
                this.currentWeather.formatted_long =
                  (Math.round(this.long * 10000) / 10000).toString() + "°E";
              } else if (this.long < 0) {
                this.currentWeather.formatted_long =
                  (-1 * (Math.round(this.long * 10000) / 10000)).toString() +
                  "°W";
              } else {
                this.currentWeather.formatted_long = (
                  Math.round(this.long * 10000) / 10000
                ).toString();
              }
...
*/
    ...
    // setFormatCoordinates: function () {
    //   console.log("inside setFormatCoordinates");
...
*/
    fixWeatherApi: function () {
      ...
      return new Promise((resolve, reject) => {
        this.getCoordinates().then((res) => {
          if (res) {
            // console.log("getCoordinates ===>", res);
            weatherApi =
              "https://csm.fusioncharts.com/files/assets/wb/wb-data.php?src=darksky&lat=" +
              res.lat +
              "&long=" +
              res.long;
            resolve((this.completeWeatherApi = weatherApi));
          } else {
            return reject();
          }
        });
    ...
    fetchWeatherData: function () {
      return new Promise((resolve, reject) => {
        this.fixWeatherApi().then((res) => {
          if (res) {
            // console.log("this.completeWeatherApi:::::", this.completeWeatherApi);
            var axios = require("axios");
            axios.get(this.completeWeatherApi).then((weatherApiResponse) => {
              // console.log(weatherApiResponse);
              if (weatherApiResponse.status === 200) {
                resolve((this.rawWeatherData = weatherApiResponse.data));
          ...
          } else console.log("Error occured when fetching fixWeatherApi");
        });
      });
    },
    ...
    getSetCurrentTime: function () {
      ...
      this.currentWeather.time = this.unixToHuman(
        this.rawWeatherData.timezone,
        this.rawWeatherData.currently.time
      ).fullTime;
    },
    ...
    getSetCurrentTemp: function () {
      if (this.rawWeatherData.currently) {
    ...
    getSetTodayTempHighLowWithTime: function () {
      ...
      var todayDetails;
      todayDetails = this.rawWeatherData.daily.data[0];
    ...
    getSetHourlyTempInfoToday: function () {
      ...
      var hourlyData = this.rawWeatherData.hourly.data;
    ...
    organizeAllDetails: async function () {
      ...
      if (
        this.rawWeatherData.currently &&
        this.rawWeatherData.daily &&
        this.rawWeatherData.hourly
      ) {
